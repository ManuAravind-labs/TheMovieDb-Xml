...
class MoviesDataSource(private val apiService: ApiService, private val type: String) :
    PagingSource<Int, MovieResult>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MovieResult> {
        return try {
            val position = params.key ?: 1
            val response = apiService.getPopularMovies(type, BuildConfig.API_KEY, position)
            LoadResult.Page(
